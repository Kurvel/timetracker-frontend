...
function Statistics() {
          ...
          setTasks((prevTasks) =>
            prevTasks.map((task) =>
              task.id === taskId
                ? {
                    ...task,
                    times: task.times.filter(
                      (timeEntry) => timeEntry.id !== timeId
                    ),
                  }
                : task
            )
  ...
  const getTimeSpentPerWeek = () => {
    ...
    tasks.forEach((task) => {
      ...
      task.times.forEach((timeEntry) => {
        ...
        timeSpentPerWeek[task.taskName][weekStartString] +=
  ...
  return (
    <div>
      ...
      <ul>
        {Object.entries(getTimeSpentPerWeek()).map(
          ([taskName, timeSpentPerWeek]) => (
            <li key={taskName}>
              <h4>{taskName}</h4>
              <ul>
                {Object.entries(timeSpentPerWeek).map(
                  ([weekStart, totalTime]) => (
                    <li key={weekStart}>
                  ...
                  (task) =>
                    ...
                    task.times.map((timeEntry) => (
                      <li key={timeEntry.id}>
                        ...
                        <button
                          onClick={() =>
                            handleDeleteTime(task.id, timeEntry.id)
                          }
                        >
                          Delete Time
                        </button>
